...
function tfs_travel_table_callback( $post ) {
    ...
    <div class="">
        <label for=""><strong><?php _e('Check the box below to use the modal table instead of expanding containers in the Rates &amp; Reservations section.'); ?></strong></label><br><br>
        <input type="checkbox" name="rr-table-checkbox" id="rr-table-checkbox" value="yes" <?php if ( isset (
            $sbm_stored_table_meta['rr-table-checkbox'])) checked($sbm_stored_table_meta['rr-table-checkbox'][0], 'yes' );?>/>
        <?php _e( 'Check box to activate rates &amp; reservations table', 'tfs-table-travel-textdomain' ); ?>
    </div>
