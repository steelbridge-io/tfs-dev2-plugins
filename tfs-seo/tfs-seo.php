...
function seotfs_custom_meta() {
		$seotypes = array('post', 'page', 'adventures', 'fishcamp_cpt', 'guide_service', 'schools_cpt', 'travel_cpt', 'espresso_venues', 'espresso_events', 'espresso_people' );
...
function seotfs_meta_save( $post_id ) {
    ...
    if( isset( $_POST[ 'seotfs-description' ] ) ) {
      update_post_meta( $post_id, 'seotfs-description', $_POST[ 'seotfs-description' ] );
		}
	
		// Checks for input and saves if needed
    if( isset( $_POST[ 'seotfs-meta-tags' ] ) ) {
      update_post_meta( $post_id, 'seotfs-meta-tags', $_POST[ 'seotfs-meta-tags' ] );
		}
